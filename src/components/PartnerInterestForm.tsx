...
export function PartnerInterestForm() {
  ...
  const fieldClass = "field-control mt-1";
  ...
  if (status === "success") {
    return <section className="card-surface border-[var(--leaf-strong)] bg-[var(--leaf)] p-6" role="status" tabIndex={-1}><h2 className="text-xl font-bold text-[var(--forest)]">Interesse registrado na demonstração</h2><p className="mt-2 text-sm text-[var(--forest-soft)]">Protocolo {protocol}. Nenhuma informação foi enviada a um serviço externo.</p><button type="button" onClick={() => setStatus("idle")} className="button-secondary mt-4">Enviar outra demonstração</button></section>;
  }

  return (
    <form onSubmit={handleSubmit} noValidate className="card-surface space-y-5 p-5 sm:p-7">
      <div><p className="eyebrow">Formulário demonstrativo</p><h2 className="mt-2 text-2xl font-bold text-[var(--forest)]">Quero conversar sobre parceria</h2><p className="mt-2 text-sm leading-6 text-[var(--muted)]">Nesta etapa, o envio é apenas simulado e não sai do navegador.</p></div>
      {Object.keys(errors).length > 0 && <div id="form-error-summary" role="alert" tabIndex={-1} className="rounded-xl border border-red-300 bg-red-50 p-3 text-sm text-red-900"><strong>Revise os campos indicados.</strong></div>}
      {status === "error" && <div role="alert" className="rounded-xl border border-red-300 bg-red-50 p-3 text-sm text-red-900"><strong>Não foi possível concluir a demonstração.</strong> Revise os dados e tente novamente.</div>}
      ...
      <div><label htmlFor="message" className="font-bold">Ideia de benefício <span className="font-normal text-[var(--muted)]">(opcional)</span></label><textarea id="message" rows={4} value={form.message} onChange={(event) => update("message", event.target.value)} className="field-control mt-1" /></div>
      ...
      <button type="submit" disabled={status === "submitting"} className="button-primary w-full disabled:cursor-wait disabled:opacity-60">{status === "submitting" ? "Simulando envio…" : "Enviar interesse demonstrativo"}</button>
    </form>
  );
}
